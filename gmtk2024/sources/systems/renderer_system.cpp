    ...
    auto renderer = std::make_shared<System>("RendererSystem", []() {
        ...
        for (auto match : matches) {
            ...
            if (model_component->hasAtlas) {
                if (model_component->isAnimated) {
                    if (model_component->elapsedFrames > model_component->animationSpeed) {
                        model_component->elapsedFrames = 0;
                        model_component->atlasIndex++;
                        if (model_component->atlasIndex >=
                                     model_component->startingIndex +
                                     model_component->animationLength ||
                                     model_component->atlasIndex < model_component->startingIndex) {
                            model_component->atlasIndex = model_component->startingIndex;
                        }
                    }
                    else {
